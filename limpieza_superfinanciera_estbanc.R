...
options(scipen = 999)
# Carpeta
carpeta <- "\\Users\\PC\\Desktop\\EQMAP\\Superfinanciera\\"
sector <- paste0(carpeta, "Bancos\\")
archivos <- list.files(sector, full.names = TRUE, pattern = "EF")
...
importar <- function(archivos) {
  ...
  for (i in seq_along(archivos)) {
    ...
    nombre <- str_match(archivos[i], "_(.*?)\\.xls")[2]
...
# Lista de los codigos CIIU
ciiu <- c("Bancos" = 6412,
          "Corporaciones" = 6421,
          "Financiamiento" = 6422,
          "Cooperativas" = 6424)
# Lectura del archivo del DIVIPOLA
divipola <- read_csv("\\Users\\PC\\Desktop\\EQMAP\\Divipola.csv",
                     col_types = "icic----",
...
lista_entidades <- read_excel(paste0(sector, "lista_bancos.xls"),
...
verificacion <- datos_limpios %>%
  ...
  mutate(ciiuID = ciiu["Bancos"])
# Paso opcional unicamente cuando faltan datos
# Compañias de Financiamiento
verificacion <- verificacion %>%
  mutate(nit = replace(nit, empresaID == 65, 860067203),
         nit = replace(nit, empresaID == 67, 860059294),
         nit = replace(nit, empresaID == 119, 900594747),
         munpioID = replace(munpioID, empresaID == 65, 11001),
         munpioID = replace(munpioID, empresaID == 67, 5001),
         munpioID = replace(munpioID, empresaID == 119, 11001))
# Aplicar NIT y nombres a los datos limpios 
datos_limpios <- datos_limpios %>%
  left_join(verificacion, by = "empresaID") %>%
  mutate(empresa = nombre) %>%
  select(-nombre, -empresa, -cuenta, -empresaID, -munpioID, -ciiuID)
