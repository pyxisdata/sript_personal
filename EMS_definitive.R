# Script para servicios mensual (EMS)
# Librerias
library(dplyr)
library(tidyr)
library(lubridate)
...
library(odbc)
# Archivos de trabajo
carpeta <- "\\Users\\PC\\Desktop\\Archivos Brutos\\Servicios Mensual\\"
# Definimos el mes y el año actual
periodo <- function(actual) {
  mes <- month(actual)
  anno <- year(actual)
  if (mes < 3) {
    anno <- anno - 1
...
url <- paste0("http://www.dane.gov.co/files/investigaciones/boletines/",
              "ems/anexos_ems_",
              meses[month(hoy)],
              substr(year(hoy), 3, 4),
              ".xls")
